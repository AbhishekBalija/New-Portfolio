		...
		window.onmousemove = (e) => {
			...
			gsap
				...
				.to(
					'.avatar',
					{
						rotation: gsap.utils.clamp(-2, 3, 5 * componentPercent.x)
					},
					0
				)
				.to(
...
<div class={clsx('relative h-full w-full', className)} bind:this={component}>
	<div class="avatar aspect-square overflow-hidden rounded-3xl border-2 border-slate-700 opacity-0">
		...
		/>
		<div
			class="highlight absolute inset-0 w-full scale-110 bg-gradient-to-tr from-transparent via-slate-50 to-transparent opacity-0"
		></div>
	</div>
</div>
