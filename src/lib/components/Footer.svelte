...
<Bounded as="footer" class="text-slate-600">
	...
	>
		<div
			class="name flex flex-col items-center justify-center gap-x-4 gap-y-2 sm:flex-row sm:justify-center"
		>
			<a
				href="/"
				class="text-xl font-extrabold tracking-tighter text-slate-100 transition-colors duration-150 hover:text-blue-400"
			>
				{settings.data.name}
			</a>
			<span
			...
			<ul class="flex items-center gap-1">
				{#each settings.data.nav_item as { link, label }, index}
					<li>
						<PrismicLink
							field={link}
							class="block px-3 py-1 text-base font-bold text-slate-100 transition-colors duration-150 hover:text-blue-400"
							>{label}</PrismicLink
						>
					</li>
		...
		<div class="socials inline-flex justify-center sm:justify-end">
			{#if isFilled.link(settings.data.github_link)}
				<PrismicLink
					...
					class="p-2 text-2xl text-slate-300 transition-all transform duration-150 hover:scale-125 hover:text-blue-400"
